...
author: "Mahendra Mariadassou<br>INRAE - MaIAGE<br>JES 2022"
...
::: columns
::: {.column width="25%"}
![Simplexe $\CS^3$, issu de [Wikipedia](https://fr.wikipedia.org/wiki/Simplexe)](figures/S3.png){fig-align="left" width="90%"}
:::
...
Le produit scalaire $<., .>_A$ induit la [**norme de Aitchison**]{.fragment .highlight-red fragment-index=3} $\parallel . \parallel_A$ et la [**distance de Aitchison**]{.fragment .highlight-red fragment-index=4} $d_A(., .)$ définies sur $\CS^D$ par:
...
<\vect{x} \oplus \alpha \odot \vect{z}, \vect{y}>_A & = \frac{1}{2D} \sum_{j=1}^D \sum_{j' = 1}^D \log \frac{x_j z_j^\alpha}{x_{j'} z_{j'}^\alpha} \log \frac{y_j}{y_{j'}} \\ 
 &= \frac{1}{2D} \sum_{j=1}^D \sum_{j' = 1}^D \left[ \log \left(\frac{x_j}{x_{j'}}\right) + \alpha \log \left(\frac{z_j}{z_{j'}}\right) \right] \log \frac{y_j}{y_{j'}} \\ 
...
On peut également définir des droites [parallèles]{.fragment .highlight-red fragment-index=2} et [orthogonales]{.fragment .highlight-red fragment-index=3}. 
...
::: {.fragment fragment-index=3}
On cherche un [**(iso)morphisme**]{.fragment .highlight-red fragment-index=3} [**isométrique**]{.fragment .fade-in fragment-index=5} de $\CS^D$ dans $\RR^D$
:::
...
$$
 \begin{align*}
  \vect{x}^\star \in \RR^{D-1} \mapsto \text{alr}_j^{-1}(\vect{x}^\star) & = \CC(e^{x_1^\star}, \dots, e^{x^\star_{j-1}}, 1, e^{x^\star_{j+1}}, \dots, e^{x^\star_D}) \\
  & = \left[ \frac{e^{x^\star_1}}{1 + \sum_{j' \neq j} e^{x^\star_{j'}}}, \dots, \frac{e^{x^\star_D}}{1 + \sum_{j' \neq j} e^{x^\star_{j'}}} \right]\in \CS^D
 \end{align*}
$$
:::
...
$$
\begin{align}
\clr(\vect{x} \oplus \vect{y}) = \text{clr}_j(\vect{x}) + \text{clr}_j(\vect{y}) & \quad \text{et} \quad \clr(\alpha \odot \vect{x}) = \alpha \text{clr}(\vect{x}) \\
...
$$
 \begin{align*}
  \vect{x}^\star \in \RR^D \mapsto \text{clr}^{-1}(\vect{x}^\star) = \CC(e^{x^\star_1}, \dots, e^{x^\star_D}) = \left[ \frac{e^{x^\star_1}}{\sum_{j=1}^D e^{x^\star_j}}, \dots, \frac{e^{x^\star_D}}{\sum_{j=1}^D e^{x^\star_j}} \right]\in \CS^D
 \end{align*}
$$
...
::: {#prp-ilr}
$\ilr_V$ est un **isomorphisme isométrique** de $\CS^D$ dans $\RR^{D-1}$ et vérifie les même propriétés que la transformation $\clr$ (cf @prp-clr)
:::
...
- $\vect{V}$ est une **matrice de contraste:** [chaque vecteur $\vect{v}_j$ vérifie $v_{j1} + \dots + v_{jD} = 0$. Le produit scalaire $\vect{v}_j^T \text{clr}(\vect{x})$ définit donc un contraste sur les composantes de $\log \vect{x}$.]{.fragment .fade-in}
...
::: {#prp-balance-lr}
### Balance
 Soit $\mathcal{T}$ un arbre binaire sur ${1, \dots, D}$. Chacun des $D-1$ noeuds internes $n_1, \dots, n_{D-1}$ définit deux sous ensembles de composantes $J_{i,g}$ et $J_{i,d}$ (liés aux sous-arbres *gauche* et *droit*). La base pivot $\vect{V} = [\vect{v}_1\, \dots\, \vect{v}_{D-1}]$ associée à $\mathcal{T}$ est définie par les vecteurs unitaires:
$$
v_{ij} = \begin{cases}
